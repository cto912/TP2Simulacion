...
class RandomNumberGenerator:
    ...
    def setup_parameters(self):
        ...
        if distribution == "Uniforme":
            ...
            ttk.Entry(self.params_frame, textvariable=self.param_a_var, width=10).grid(row=0, column=1, padx=(5, 10),
                                                                                       pady=2)
            ...
            ttk.Entry(self.params_frame, textvariable=self.param_b_var, width=10).grid(row=0, column=3, padx=(5, 0),
                                                                                       pady=2)
            ...
            ttk.Entry(self.params_frame, textvariable=self.param_lambda_var, width=10).grid(row=0, column=1,
                                                                                            padx=(5, 0), pady=2)
            ...
            ttk.Entry(self.params_frame, textvariable=self.param_mu_var, width=10).grid(row=0, column=1, padx=(5, 10),
                                                                                        pady=2)
            ...
            ttk.Entry(self.params_frame, textvariable=self.param_sigma_var, width=10).grid(row=0, column=3, padx=(5, 0),
                                                                                           pady=2)
            ...
            with pd.ExcelWriter(file_path, engine='openpyxl') as writer:
                ...
                stats_data = {
                    ...
                    'Valor': [
                        ...
                        np.mean(self.generated_numbers),
                        np.std(self.generated_numbers, ddof=1),
                        min(self.generated_numbers),
                        max(self.generated_numbers),
                        np.median(self.generated_numbers),
                        np.percentile(self.generated_numbers, 25),
                        np.percentile(self.generated_numbers, 75)
                    ],
                }
                ...
                if distribution == 'Uniforme':
                    ...
                    a_val = self.generation_parameters.get('a', 'N/A')
                    b_val = self.generation_parameters.get('b', 'N/A')
                    try:
                        a_val = float(a_val) if a_val != 'N/A' else a_val
                        b_val = float(b_val) if b_val != 'N/A' else b_val
                    except (ValueError, TypeError):
                        pass
                    stats_data['Valor'].extend([a_val, b_val])
                elif distribution == 'Exponencial':
                    stats_data['Estadística'].append('Parámetro λ (lambda)')
                    lambda_val = self.generation_parameters.get('lambda', 'N/A')
                    try:
                        lambda_val = float(lambda_val) if lambda_val != 'N/A' else lambda_val
                    except (ValueError, TypeError):
                        pass
                    stats_data['Valor'].append(lambda_val)
                elif distribution == 'Normal':
                    stats_data['Estadística'].extend(['Parámetro μ (media)', 'Parámetro σ (desv. est.)'])
                    mu_val = self.generation_parameters.get('mu', 'N/A')
                    sigma_val = self.generation_parameters.get('sigma', 'N/A')
                    try:
                        mu_val = float(mu_val) if mu_val != 'N/A' else mu_val
                        sigma_val = float(sigma_val) if sigma_val != 'N/A' else sigma_val
                    except (ValueError, TypeError):
                        pass
                    stats_data['Valor'].extend([mu_val, sigma_val])
